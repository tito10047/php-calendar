...
Combine the CalDAV server with a `LiveComponent` for reactive month navigation:

### Component class

```php
// src/Twig/Components/CalendarWidget.php
...
```twig
{# templates/components/CalendarWidget.html.twig #}
<div {{ attributes.defaults({class: 'calendar-widget'}) }}>
    <header class="calendar-header">
        <button data-action="live#action" data-live-action-param="prevMonth">‹</button>
        <h2>{{ this.calendar.date|date('F Y') }}</h2>
        <button data-action="live#action" data-live-action-param="nextMonth">›</button>
    </header>

    <div class="calendar-grid">
        {% set table = this.calendar.getDaysTable() %}
        {% for week in table %}
            <div class="week">
                {% for day in week %}
                    <div class="day
                        ...
                        {% if not day.ghost %}
                            ...
                            {% for event in day.data ?? [] %}
                                <div class="event" style="background: {{ event.color ?? '#3498db' }}">
                                    {{ event.summary }}
                                </div>
                            {% endfor %}
                        {% endif %}
...
Navigation (prev/next month) re-renders only the component — no full page reload. Events saved via the CalDAV endpoint (from Apple Calendar or a FullCalendar UI) appear on the next render automatically because both sides share the same `DatabaseCalendarStore`.
...
class CalendarWidget extends Component
{
    ...
    public function render(DatabaseCalendarStore $store): \Illuminate\View\View
    {
...
```blade
{{-- resources/views/livewire/calendar-widget.blade.php --}}
<div>
...
Navigating between months sends a single Livewire request — no JSON API, no JavaScript fetch, no state management library.
